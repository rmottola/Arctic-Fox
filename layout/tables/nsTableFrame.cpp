...
nsTableFrame::GetOuterBCBorder() const
{
  ...
  if (propData) {
    border.top = BC_BORDER_START_HALF_COORD(d2a, propData->mTopBorderWidth);
    border.right = BC_BORDER_END_HALF_COORD(d2a, propData->mRightBorderWidth);
    border.bottom = BC_BORDER_END_HALF_COORD(d2a, propData->mBottomBorderWidth);
    border.left = BC_BORDER_START_HALF_COORD(d2a, propData->mLeftBorderWidth);
  }
  return border;
}
    ...
    for (nsTableRowFrame* rowFrame = rgFrame->GetFirstRow(); rowFrame;
      ...
      if (haveIntersect) {
        ...
        nscoord borderHalf = mTable->GetPrevInFlow() ? 0 :
			         ...
			         rowFrame->GetTopBCBorderWidth() + 1);
        ...
        nscoord borderHalf = mTable->GetNextInFlow() ? 0 :
	                       presContext->DevPixelsToAppUnits(
			         rowFrame->GetBottomBCBorderWidth() + 1);
                       ...
                       BCPixelSize          aHorSegHeight)
{
  ...
  nscoord offset = CalcHorCornerOffset(cornerOwnerSide, cornerSubWidth,
                                       ...
                                       aIter.mTableWM.IsBidiLTR(), aIter.mTable->PresContext());
                                ...
                                BCPixelSize            aLeftSegWidth)
{
  ...
  mEndOffset = CalcHorCornerOffset(ownerSide, cornerSubWidth, verWidth, false,
                                   mIsRightBevel, aIter.mTableWM.IsBidiLTR(), aIter.mTable->PresContext());
  mLength += mEndOffset;
