...
BytecodeEmitter::emitNormalFor(ParseNode *pn, ptrdiff_t top)
{
    ...
    if (!init) {
        ...
        emittingForInit = true;
        if (!UpdateSourceCoordNotes(cx, this, init->pn_pos.begin))
            return false;
        if (!EmitTree(cx, this, init))
            return false;
        emittingForInit = false;
    ...
    if (ParseNode *update = forHead->pn_kid3) {
        if (!UpdateSourceCoordNotes(cx, this, update->pn_pos.begin))
            return false;
        op = JSOP_POP;
        if (!EmitTree(cx, this, update))
            return false;
