...
private:
  ...
  RefPtr<MediaEngineSource> mAudioSource; // threadsafe
  RefPtr<MediaEngineSource> mVideoSource; // threadsafe
...
public:
  ...
  {
    if (mSourceStream) {
      RefPtr<dom::MediaStreamTrack> track = GetDOMTrackFor(aTrackID);
      if (track) {
